...
- 功能默认关闭。启用后统一处理群聊和私聊中的图片，不设置群白名单、私聊许可或其他消息范围开关；图片无 GPS、位置不可用或处理阶段失败时静默跳过回复。
- 位置服务同时保留 Nominatim 兼容接口和高德开放平台，不再配置服务商选择。有高德 Key 时先用高德，地址不可用或请求失败时以原始 WGS-84 坐标回退 Nominatim；无 Key 时直接用 Nominatim，旧 `provider` 配置不影响调用顺序。默认 Nominatim endpoint 为 OSMF 官方 reverse API，不得要求用户自建或禁用官方接口；自定义 endpoint 只接受 HTTPS。高德使用官方 HTTPS 接口、Web 服务 Key 数组和同提供商内的可重试轮换，并在请求前将 WGS-84 坐标转换为 GCJ-02。
- 支持普通图片消息和以文件消息发送的 HEIF/HEIC。文件消息须通过适配器 `get_file` 获取临时 URL 或受限本地缓存，不信任消息段携带的本地路径；本地读取只接受常规绝对路径，拒绝 UNC 和 Windows 设备命名空间，并同时限制等待时间、文件大小和并发数。可取消的文件读取在超时后应主动终止。
- 回复保持 `请问是<位置>的<昵称> <称谓>吗？` 的自然问句。昵称优先使用群名片，其次使用发送者昵称，最后使用中性回退值；称谓允许配置但不得推断性别。不得附加“位置信息”、位置服务商或类似署名，也不要为此类署名新增配置。
- 日志沿用 `components/logger.js` 和 `[图片EXIF]` 前缀，使用自然中文描述，并按 `debug`、`info`、`warn`、`error` 区分任务状态、处理阶段、耗时、服务商和安全失败原因。不要使用 `provider=nominatim`、`stage=download` 一类面向机器的键值串代替可读文案；不得记录图片 URL、本地路径、坐标、解析地点、昵称、API Key 或完整外部响应。
- 图片、EXIF、坐标和地理编码响应只在内存中处理，不落盘、不建立包含敏感位置的持久缓存。外部请求继续使用超时、队列、限流、容量有界的短期缓存和统一代理开关；介绍公共服务时应引用其现行政策或定价，不承诺免费、无限额度或可用性。
- 新增或调整该功能时，同步检查锅巴配置卡片、默认配置、配置契约测试和帮助菜单。单元测试通过注入 `fetch`、文件读取器和时钟覆盖双服务商、普通图片、HEIF/HEIC、无 GPS、超时、大小限制、路径边界、日志脱敏和静默失败，不访问真实网络或 Bot。
