...
- 功能默认关闭。启用后统一处理群聊和私聊中的图片，不设置群白名单、私聊许可或其他消息范围开关；图片无 GPS、位置不可用或任一阶段失败时静默跳过回复。
- 位置服务同时保留 Nominatim 兼容接口和高德开放平台。默认使用 OSMF 官方 Nominatim reverse API，不得要求用户自建或禁用官方接口；自定义 Nominatim endpoint 只接受 HTTPS。高德使用官方 HTTPS 接口、Web 服务 Key 数组和同提供商内的可重试轮换，并在请求前将 WGS-84 坐标转换为 GCJ-02。
